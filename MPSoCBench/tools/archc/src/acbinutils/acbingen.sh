...
if [ -z "$BINUTILS_PATH" ]; then
  BINUTILS_PATH=`grep BINUTILS_PATH /home/felippi/tools2/MPSoCBench/tools/archc/etc/archc.conf | cut -d = -f2`
...
mkdir -p $TEMP_DIR
[ $? -ne 0 ] && exit $?
cp -rf /home/felippi/tools2/MPSoCBench/tools/archc/share/archc/binutils ${TEMP_DIR}/


# generate the files in the binutils tree
echo "Generating machine dependent code..."
/home/felippi/tools2/MPSoCBench/tools/archc/bin/bmdsfg $MODEL_FILE -a${ARCH_NAME}
[ $? -ne 0 ] && exit $?
...
if [ -z "$GDB_PATH" ]; then
  GDB_PATH=`grep GDB_PATH /home/felippi/tools2/MPSoCBench/tools/archc/etc/archc.conf | cut -d = -f2`
...
mkdir -p $TEMP_DIR
[ $? -ne 0 ] && exit $?
cp -rf /home/felippi/tools2/MPSoCBench/tools/archc/share/archc/gdb ${TEMP_DIR}/
